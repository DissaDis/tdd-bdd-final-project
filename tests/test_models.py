...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_a_product(self):
        ...
        self.assertEqual(product.description, "testingus")
        # Fetch all the product back from the system.
        # Assert the length of the products list is equal to 1
        # to verify that after updating the product, there is only one product in the system.
        ...
        self.assertEqual(products[0].description, "testingus")
    ...
    def test_delete_a_product(self):
        ...
        # Product.all() is equal to 1, to verify that after
        # creating a product and saving it to the database, there is only one product in the system.
        products = Product.all()
        self.assertEqual(len(products), 1)
        # Call the delete() method on the product object, to remove the product from the database.
        product.delete()
        # Assert if the length of the list returned by
        # Product.all() is now equal to 0, indicating that the product has been successfully deleted from the database.
    ...
    def test_list_all_products(self):
        ...
        # there are no products in the database at the beginning of the test case.
        ...
        products = Product.all()
        # Assert if the length of the products list is equal to
        # 5, to verify that the five products created in the previous step have been successfully added to the database.
    ...
    def test_find_by_price_sad(self):
        """It should Find a Product by Price even price is a string"""
        products = ProductFactory.create_batch(5)
        for product in products:
            product.create()
        price = products[0].price
        count = len([product for product in products if product.price == price])
        price_new = ' "' + str(products[0].price) + '" '
        found = Product.find_by_price(price_new)
        self.assertEqual(found.count(), count)
        for product in found:
            self.assertEqual(product.price, price)
